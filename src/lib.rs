...
// jd: 相对于儒略日的天数
fn calc_y(jd: f64) -> YearCalender {
    let j2000 = EPHCONST::J2000;
    let mut year_info = YearCalender::new(AstroyDate::jd2day(jd + j2000).0);

    let mut a: &mut [f64; 25] = &mut year_info.zq;
    let mut b = &mut year_info.hs;
    let mut w: f64;
    ...
    for i in 0..14{
        ...
        if is_leap==1{
            year_info.ym[i as usize] = format!("闰{}",  EPHCONST::YMC[(lm  % 12) as usize]);

        }else{
            year_info.ym[i as usize] = format!("{}",  EPHCONST::YMC[(lm  % 12) as usize]);
        }
...
fn test_convert_to_lunar(){
     ...
     let m = 11;
     let d=19;
     let jd2 = AstroyDate::from_day(y, m, d as f64 + 0.5).jd - EPHCONST::J2000;
     ...
     println!("公历：{}年{}月{}日  农历:{}年 {}月 {}日", y, m, d, 
     yi.year, yi.ym[i-1], (jd2- yi.hs[(i -1)] +1.0) as i32);
...
impl AstroyDate {
    ...
    fn from_day(mut y: i32, mut m: i32, d: f64) -> Self {
        let mut n = 0;
...
fn calc(mut jd: f64, qs: &str) -> f64 {
    let mut jd = jd + 2451545.0;
    let mut i;
    let mut d: i32;
    let mut n;
    let mut b = EPHCONST::SUO_KB.to_vec();
    let mut pc = 14;
    if qs == "气" {
        b = EPHCONST::QI_KB.to_vec();
        pc = 7;
    }
...
fn dt_calc(y: f64) -> f64 {
    let dt_at_len = EPHCONST::DT_AT.len();
    let y0 = EPHCONST::DT_AT[dt_at_len - 2]; // 表中最后一年
    let t0 = EPHCONST::DT_AT[dt_at_len - 1]; // 表中最后一年的deltatT
    if y >= y0 {
...
fn so_high(w: f64) -> f64 {
    let t = moon_a_lon_t2(w) * 36525.0;
    let t = t - dt_t(t) + 8.0 / 24.0;
    let v = ((t + 0.5) % 1.0) * 86400.0;
    let mut result = t;
    if v < 1800.0 || v > 86400.0 - 1800.0 {
        result = moon_a_lon_t(w) * 36525.0 - dt_t(t) + 8.0 / 24.0;
    }
    result
}

fn so_accurate(w: f64) -> f64 {
    let t = moon_a_lon_t(w) * 36525.0;
    t - dt_t(t) + 8.0 / 24.0
}

fn so_accurate2(jd: f64) -> f64 {
...
fn qi_hight(w: f64) -> f64 {
    let mut t = solor_a_lon_t2(w) * 36525.0;
    t = t - dt_t(t) + 8.0 / 24.0;
    let v = ((t + 0.5) % 1.0) * 86400.0;
    if v < 1200.0 || v > 86400.0 - 1200.0 {
        t = solor_a_lon_t(w) * 36525.0 - dt_t(t) + 8.0 / 24.0;
    }
    t
}
...
fn qi_accurate(w: f64) -> f64 {
    let t = solor_a_lon_t(w) * 36525.0;
    t - dt_t(t) + 8.0 / 24.0
}

fn qi_accurate2(jd: f64) -> f64 {
    let d = PI / 12.0;
...
fn test_qi_accurrate2() {
    let jd = 341.0;
    let exp = 340.65072186631636;
    let r = qi_accurate2(jd);
    println!("exp={}, r={}, r-exp={}", exp, r, (r - exp) * (10.0_f64).powi(6));
    assert!((r - exp).abs() < 1e-6)
}
...
fn nutation2(t: f64) -> (f64, f64) {
    ...
    (dl / 100.0 / EPHCONST::RAD, de / 100.0 / EPHCONST::RAD)
}
...
fn gxc_sun_lon(t: f64) -> f64 {
    ...
    (-20.49552 * (1.0 + e * v.cos())) / EPHCONST::RAD
}

fn gxc_moon_lon(t: f64) -> f64 {
    -3.4e-6
} //月球经度光行差
...
#[test]
fn test_moon_lon() {
    let exp = -4658.341531724926;
    let t = -0.555026229290277;
    let mn = 10;
    let r = moon_lon(t, mn);
    println!("r={}, e={}", r, (r - exp).abs());
    assert!((r - exp).abs() < 1e-6);
}
...
fn solor_a_lon_t2(w: f64) -> f64 {
    ...
    t -=
        ...
        v;
    t +=
        (w - earth_lon(t, 8) - PI + (20.5 + 17.2 * (2.1824 - 33.75705 * t).sin()) / EPHCONST::RAD) /
        v;
    t
}
...
fn eph_calc(xt: usize, zn: usize, mut t: f64, mut n: i32) -> f64 {
    let mut t = t / 10.0; // 转为儒略千年数
    let mut tn = 1.0;
    let mut v = 0.0;

    let f: &Vec<f64> = &EPHCONST::XL0[xt];
    let pn = zn * 6 + 1;
    let mut n1: i32;
    let mut n2: i32;
    let mut n0: i32;
    let mut n_: i32;
    let mut n_0 = (f[pn + 1] - f[pn]) as i32; // N0 序列总数
    ...
    if xt == 0 {
        ...
        match zn {
            0 => {
                v += (-0.0728 - 2.7702 * t - 1.1019 * t2 - 0.0996 * t3) / EPHCONST::RAD;
            }
            1 => {
                v += (0.0 + 0.0004 * t + 0.0004 * t2 - 0.0026 * t3) / EPHCONST::RAD;
            }
            2 => {
        ...
        // 其它行星
        let dv = EPHCONST::XL0_XZB[(xt - 1) * 3 + zn];
        match zn {
            0 => {
                v -= (3.0 * t) / EPHCONST::RAD;
            }
            2 => {
                v += dv / 1000000.0;
            }
...
// 月亮星历法计算
fn xl1_calc(zn: usize, t: f64, mut n: i32) -> f64 {
    let ob = &EPHCONST::XL1[zn];
    let mut v = 0.0;
    let mut tn = 1.0;
    let mut t2 = t * t;
    let mut t3 = t2 * t;
